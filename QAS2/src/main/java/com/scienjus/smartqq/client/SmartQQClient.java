...
public class SmartQQClient implements Closeable
{
	...
	{
		this.client = Client.pooled().maxPerRoute(8).maxTotal(15).build();
	...
	private void getQRCode()
	{
		...
		{
			filePath = new File(
					"D:\\Tomcat\\apache-tomcat-8.0.39-windows-x86\\apache-tomcat-8.0.39\\webapps\\QAS\\qrcode.png")
							.getCanonicalPath();
		} catch (IOException e)
		{
			throw new IllegalStateException("二维码保存失败");
		}
		Response<File> response = session.get(ApiURL.GET_QR_CODE.getUrl()).addHeader("User-Agent", ApiURL.USER_AGENT)
				.file(filePath);
